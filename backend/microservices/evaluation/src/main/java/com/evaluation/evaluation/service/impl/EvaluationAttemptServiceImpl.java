...
public class EvaluationAttemptServiceImpl implements EvaluationAttemptService {
    ...
    @Override
    public EvaluationAttempt startAttempt(Long evaluationId, Long studentId) {
        ...
        attempt.setAttemptNumber(getAttemptCount(studentId, evaluationId) + 1);
    ...
    private int getAttemptCount(Long studentId, Long evaluationId) {
        return evaluationAttemptRepository.findByStudentIdAndEvaluationId(studentId, evaluationId).size();
    }
        ...
        for (StudentAnswer answer : attempt.getStudentAnswers()) {
            ...
            switch (question.getQuestionType()) {
                ...
                case MSQ:
                    ...
                    // detailed logic needed, for now simplified:
                    // If we had full comparison logic.
                    // Let's check if all selected are correct.
                    if (answer.getSelectedOptions() != null && !answer.getSelectedOptions().isEmpty()) {
                        boolean allSelectedAreCorrect = answer.getSelectedOptions().stream()
                                .allMatch(com.evaluation.evaluation.model.Option::getIsCorrect);
                        // And we need to ensure ALL correct options were selected to get full points?
                        // Or partial points? Let's assume strict grading for now: all correct ones must
                        // be selected.
                        // But we don't have easy access to "all correct options" without fetching
                        // Question.options.
                        // Let's stick to: "Score += points if (selected == correct_options)"
                        // For simplicity in this iteration: If all selected are correct, give points.
    ...
    @Override
    public List<EvaluationAttempt> getAttemptsByStudentAndEvaluation(Long studentId, Long evaluationId) {
        return evaluationAttemptRepository.findByStudentIdAndEvaluationId(studentId, evaluationId);
    }
}
